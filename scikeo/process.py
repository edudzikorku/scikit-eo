...
def confintervalML(matrix, image_pred, pixel_size = 10, conf = 1.96, nodata = None):
    ...
    # producer's accuracy
    pa = np.diag(matConf) / np.sum(matConf, axis=0)
    N_j = np.sum(matrix, axis=0)
    # variance of producer's accuracy
    V_PA = pa * (1 - pa) / (N_j - 1)
    # confidence interval for producer's accuracy at 95%
    conf_int_pa = conf * np.sqrt(V_PA)
    conf_int_pa[np.isnan(conf_int_pa)] = 0
    ...
    result = {'Overall_accuracy':oa,
              ...
              'Producers_accuracy_95%': conf_int_pa,
